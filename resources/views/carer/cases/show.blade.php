...
<div x-data="{ tab: 'child' }" class="space-y-4">
    ...
    {{-- ── Wellbeing ── --}}
    <div x-show="tab==='wellbeing'" class="bg-white p-6 rounded-xl border border-gray-100 space-y-4">
        <h2 class="text-base font-semibold text-gray-900">Wellbeing checks</h2>

        @forelse(($case->wellbeingChecks ?? collect())->sortByDesc('completed_at')->take(10) as $check)
            @php
            ...
            <div class="border border-gray-100 rounded-lg p-4">
                <div class="flex items-center justify-between mb-3">
                    <p class="text-sm font-medium text-gray-900">
                        {{ $check->completed_at?->format('d M Y') ?? $check->created_at->format('d M Y') }}
                        @if($loop->first)<span class="ml-2 text-xs text-indigo-600">(Latest)</span>@endif
                    </p>
                    <div class="flex items-center gap-2">
                        <span class="text-xs {{ $badgeCls }} px-2 py-0.5 rounded-full font-medium">{{ ucfirst($wRisk) }}</span>
                        <span class="text-sm font-semibold text-gray-700">{{ round($check->overall_score ?? 0) }}/100</span>
                    </div>
                </div>

                {{-- Domain scores from the pivot table --}}
                @if($check->domainScores && $check->domainScores->count() > 0)
                    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-x-6 gap-y-1.5">
                        @foreach($check->domainScores as $ds)
                            @php
                            ...
                            <div>
                                <div class="flex items-center justify-between mb-0.5">
                                    <span class="text-[10px] text-gray-500 truncate">{{ $ds->domain->name ?? 'Domain' }}</span>
        ...
        @empty
            <p class="text-sm text-gray-500">No wellbeing checks completed yet.</p>
        @endforelse
